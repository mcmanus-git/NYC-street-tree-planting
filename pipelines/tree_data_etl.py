...
def get_most_recent_upload_date(in_response_text: str) -> datetime.date:
    last_update = datetime.strptime(in_response_text.replace('"', '').split('\n')[5].split(',')[1], '%Y-%m-%d %H:%M:%S').date()
    return last_update


def process_new_data(in_response_text: str):
    # Split the text into lines
    lines = in_response_text.replace('"', '').split('\n')
    ...
    out_df['CompletedDate'] = pd.to_datetime(out_df['CompletedDate'])
    out_df['PlantingSeason'] = pd.to_datetime(out_df['PlantingSeason'])
    out_df['lng'] = out_df['lng'].astype(float)
    out_df['lat'] = out_df['lat'].astype(float)
...
def load_tree_data(save=True, return_data=True):
    ...
    if os.path.exists(today_file):
        ...
        web_data = download_new_data()
        most_recent_web = get_most_recent_upload_date(web_data)
        if most_recent_web == today:
            df = process_new_data(web_data)
            if save:
...
#     # https://www.nycgovparks.org/tree-work-orders/street_tree_planting.csv
#     # https://www.nycgovparks.org/trees/street-tree-planting/locations#:~:text=a%20particular%20address.-,Download%20The%C2%A0Data,-Show
#
